...
All four notebooks default their sample text/URN to `syntax_model.md`'s own worked examples -- most of which are drawn from Lysias 1, *On the Murder of Eratosthenes* (`urn:cts:greekLit:tlg0540.tlg001.perseus-grc2:`) -- as a placeholder; point `urnbase` at your own corpus. Run any of them with `marimo edit marimo/syntaxer.py` (requires `pip install marimo --break-system-packages`) from the repo root, and a `.env` with `API_BASE`/`MODEL`/`API_KEY` set (see DEVELOPMENT.md), except `syntaxer_review.py`, which needs no LM access at all.
