...
- `syntaxer_main.py` — command-line entry point: loads `.env`, configures the LM, and runs an analysis for a passage given on the command line. The one runnable script that stays at the repo root; every other one lives under `utilities/` (below).
- `utilities/` — runnable scripts other than `syntaxer_main.py` (CLI tools and live-LM scripts, as opposed to the `grammatike/` package they all import from):
  ...
  - `optimize_gepa.py` — GEPA optimization script (see OPTIMIZING.md).
  - `model_bakeoff.py` — "bakeoff" utility comparing candidate models (see BAKEOFF.md).
  - `analysis_to_dot.py` / `analyses_to_png.py` — Graphviz DOT/PNG rendering CLIs for saved analyses (see "Graphviz DOT diagrams" / `notes/dot_diagrams.md`).
  - `build_api_docs.py` — regenerates `docs/grammatike-api-docs.html`, a single self-contained HTML page documenting every name in `grammatike.__all__`, built with `pdoc` straight from the package's own docstrings and type hints. Run `python utilities/build_api_docs.py` after changing a public docstring or signature to refresh it; requires `pdoc` (`pip install pdoc --break-system-packages`). Its output stays in `docs/` even though the script itself lives here.
- `grammatike/` — the package with the actual analysis logic:
  ...
  - `gepa_metric.py` — `syntax_metric()`, the scoring/feedback function used by `utilities/optimize_gepa.py` and `utilities/model_bakeoff.py` (see OPTIMIZING.md/BAKEOFF.md).
